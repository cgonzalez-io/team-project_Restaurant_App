...
public class StaffAssignmentRepositoryImpl implements StaffAssignmentRepository {
    ...
    public void insert(StaffAssignment staffAssignment) throws SQLException {
        String sql = "INSERT INTO ASSIGNED_TO (Table_ID, Staff_ID, Date, Time) " + "VALUES (?, ?, ?, ?)";
    ...
    @Override
    public void update(StaffAssignment oldAssignment, StaffAssignment newAssignment) throws SQLException {
        if(oldAssignment == null || newAssignment == null) {
            throw new SQLException("Assignment does not exist.");
        }
        //delete orignal assigment
        delete(oldAssignment.getTableID(), oldAssignment.getStaffID(), oldAssignment.getDate(), oldAssignment.getTime());
