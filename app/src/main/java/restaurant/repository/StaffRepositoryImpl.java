...
public class StaffRepositoryImpl implements StaffRepository {
  ...
  public void update(Staff staff) throws SQLException {
    String sql = "UPDATE STAFF SET First_Name = ?, Last_Name = ?, Role = ?, "
                 +
    ...
    try (Connection conn = DatabaseUtil.getConnection();
      ...
      if (affectedRows == 0) {
        throw new SQLException("Staff with ID " + staff.getStaffId() +
                               " does not exist.");
      }
    }
  }
  ...
  public List<Staff> findAll() throws SQLException {
    ...
    try (Connection conn = DatabaseUtil.getConnection();
      ...
      while (rs.next()) {
        staffList.add(
            new Staff(rs.getInt("Staff_ID"), rs.getString("First_Name"),
